...
`cinder-gfx-scene-flow` is centered on graphics work in Dart. The technical target is: Design a Dart verification harness for scene systems, covering visual model generation, layout fixtures, and failure-oriented tests.
...
The score is deterministic. The current threshold is 158. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 6.
